class TicTacToe {
    ...
    turn(player = 0, x = 0, y) {
        if (this.board === 511 || this._winner) return -3
        let pos = 0
        if (y == null) {
            if (x < 0 || x > 8) return -1
            pos = 1 << x
        ...
        if (TicTacToe.check(this._x)) this._winner = this.playerX
        if (TicTacToe.check(this._o)) this._winner = this.playerO
    ...
    surrender(byPlayer) {
        this._winner = byPlayer === this.playerX ? this.playerO : this.playerX
